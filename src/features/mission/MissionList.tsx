...
export function MissionList() {
    ...
    const [searchResult, setSearchResult] = useState<MissionState[]>([]);
    const printMission = (list: MissionState[]) => {
        return list.map((missionData: MissionState) => {
            return <Grid key={missionData.id} item xs={6} md={4}><Mission {...missionData}  /></Grid>
        });
    }
                ...
                <Card variant="outlined" className="searchInput">
                    <FormControl fullWidth>
                        ...
                        <OutlinedInput
                            ...
                            label="Search"
                            onChange={(e) => {
                                dispatch(searchMission({text: e.target.value}));
                    ...
                    <FormControl fullWidth>
                        ...
                        >
                            ...
                            <ToggleButton value={"dsa"} onClick={() => dispatch(sortMissions({
                                attribute: 'id',
                                func: (a: any, b: any) => (b - a)
                            }))}>
                                Sort By ID
                            </ToggleButton>
                            <ToggleButton value="ss" onClick={() => dispatch(sortMissions({
                                attribute: 'date',
                                func: (a: any, b: any) => (a - b)
                            }))}>
                                Sort By Date
                            </ToggleButton>
                            <ToggleButton value={"da"} onClick={() => dispatch(sortMissions({
                                attribute: 'priority',
                                func: (a: any, b: any) => (a - b)
                            }))}>
                                Sort By Priority
                            </ToggleButton>
                            <ToggleButton value="31" onClick={() => dispatch(sortMissions({
                                attribute: 'status',
                                func: (a: any, b: any) => (a - b)
                            }))}>
                                Sort By Status
